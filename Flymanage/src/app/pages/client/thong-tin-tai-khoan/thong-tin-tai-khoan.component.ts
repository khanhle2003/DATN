    ...
    this.otpService.verifyOtp(otpRequest).subscribe({
      next: (response) => {
        ...
        if (response && response.success) {
          // Xác thực OTP thành công, tiếp tục với đổi mật khẩu
          ...
          this.userService.changePassword(changePasswordRequest).subscribe({
            ...
            error: (pwError) => {
              ...
              // Nếu lỗi là lỗi parse, giả định đổi mật khẩu thành công
      ...
      error: (error) => {
        ...
        // Nếu lỗi là lỗi parse, giả định xác thực thành công
        if (error.message && error.message.includes('Http failure during parsing')) {
          // Tiếp tục với đổi mật khẩu
          const changePasswordRequest = {
            username: this.user.username,
            oldPassword: this.passwordForm.get('oldPassword')?.value,
            newPassword: this.passwordForm.get('newPassword')?.value
